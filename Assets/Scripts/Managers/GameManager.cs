...
// Agrega método para skipear tutorial desde UI
    ...
    private void createSlotsItems(int quantityItems)
    {
        for (int i = 0; i < 4; i++)
        {
            GameObject gObject = Instantiate(ItemsObject[i], inventoryContent);
            itemsImage.Add(gObject.GetComponent<Image>());
            quantityText.Add(gObject.transform.GetChild(0).GetComponent<TextMeshProUGUI>());
            if (itemInventory[i].Quantity == 0)
            {
                itemsImage[i].enabled = false;
                quantityText[i].enabled = false;
            }
            else quantityText[i].text = itemInventory[i].Quantity.ToString();
        }
    ...
    public void QuitItem(int quantityItems)
    {
        ...
        {
            currentIndex = i;
            itemsImage[currentIndex].sprite = itemsSprites[currentIndex];
